...
/**
 * Calculate rentability (RENT) for a date range and all shops
 * Usage: node calculateRentabilityRange.js "2026-01-01" "2026-02-19"
 */

async function calculateRentabilityRange(startDate, endDate) {
    try {
        ...
        console.log(`💰 Iniciando cálculo de RENTABILIDAD`);
        ...
        while (currentDate <= end) {
            ...
            // Calculate rentability for each shop
            for (const shop of shops) {
                try {
                    await UpdateRentabilityUseCase.execute(shop._id, dateStr);
                    console.log(`  ✅ Rentabilidad calculada para tienda ${shop._id}`);
                    success++;
                } catch (err) {
        ...
        console.log(`\n🎉 Cálculo de rentabilidad completado: ${success} éxitos, ${errors} errores`);
        process.exit(0);
    } catch (err) {
...
if (!startDate || !endDate) {
    console.error('❌ Uso: node calculateRentabilityRange.js "YYYY-MM-DD" "YYYY-MM-DD"');
    console.error('Ejemplo: node calculateRentabilityRange.js "2026-01-01" "2026-02-19"');
    process.exit(1);
}

calculateRentabilityRange(startDate, endDate);
